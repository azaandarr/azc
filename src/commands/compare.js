...
// ServiceAlias is matched case-insensitively against the sku-mappings aliases.
...
module.exports = function registerCompareCommand(program) {
  program
    ...
    .requiredOption('-w, --with <spec>', 'Change spec, e.g. "App Service:P1v3" or "PostgreSQL:Standard_D4ds_v5"')
    ...
    .action(async (opts) => {
      ...
      // ── Get proposed cost ───────────────────────────────────────
      // Look up the new SKU's price using the same service name
      ...
      try {
        ...
        const skuLower = change.newSku.toLowerCase().replace(/\s+/g, '');
        ...
        // Apply instance count from the change props or current config
        const instances = change.props.instances
          ? parseInt(change.props.instances, 10)
      ...
      const compareData = {
        ...
        proposed: { sku: change.newSku, monthlyCost: proposedMonthlyCost },
        currency: opts.currency,
      };
